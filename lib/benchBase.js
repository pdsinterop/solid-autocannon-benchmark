...
const { generateProof, generateKeyPair } = DPoP;

export class benchBase {
  constructor(options) {
    ...
    this.consentPath = options.consentPath ? options.consentPath : process.env.SOLID_CONSENT_PATH;
    ...
    this.tokenPostType = options.tokenPostType ? options.tokenPostType : process.env.SOLID_TOKEN_POST_TYPE;
    this.tokenData = {
      "client_id" : "",
      "redirect_uri" : this.redirectUri,
    ...
    this.keyPair = generateKeyPair('ES256');
  }
  ...
  getConsentBody() {
    this.consentPostData.client_id = this.clientRegistration.client_id;
  ...
  async getAuthorizeCode() {
    let keyPair = await this.keyPair;
  ...
  getTokenBody() {
    ...
    this.tokenData['client_secret'] = this.clientRegistration.client_secret;
  ...
  async getTokenHeaders() {
    let keyPair = await this.keyPair;
    const dpopHeader = await generateProof(keyPair, this.url + this.tokenPath, this.tokenMethod);
    switch (this.tokenPostType) {
      case 'form':
        return {
          'Cookie' : this.cookie,
  ...
  async getProfileHeaders() {
    let keyPair = await this.keyPair;
    const nonce = undefined; // null is not acceptible for dpop()
    const dpopHeader  = await generateProof(keyPair, this.profileUrl, 'GET', nonce, this.token['access_token']);
    return {
      "Authorization" : "DPoP " + this.token['access_token'],
      "DPoP" : dpopHeader
    };
  }
  ...
  async getStorageHeaders(targetUrl) {
    let keyPair = await this.keyPair;
    const nonce = undefined; // null is not acceptible for dpop()
    const dpopHeader  = await generateProof(keyPair, targetUrl, 'GET', nonce, this.token['access_token']);
    return {
      "Authorization" : "DPoP " + this.token['access_token'],
      "DPoP" : dpopHeader
    };
